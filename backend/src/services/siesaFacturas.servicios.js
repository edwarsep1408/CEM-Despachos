...
const configFacturas = () => {
  const key = process.env.SIESA_FACTURAS_CONNI_KEY || process.env.SIESA_PEDIDOS_CONNI_KEY || "";
  const token = process.env.SIESA_FACTURAS_CONNI_TOKEN || process.env.SIESA_PEDIDOS_CONNI_TOKEN || "";
  const rawUrl =
